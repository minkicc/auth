# Auth Go Resource Server Example

这个示例演示的是业务资源服务如何在 OIDC-first 分支里校验 Auth 发出的 access token。
...
AUTH_EXPECTED_AUDIENCE=demo-backend
AUTH_REQUIRED_SCOPE=
LISTEN_ADDR=:8083
```
...
- 如果你把 `AUTH_REQUIRED_SCOPE` 设成某个 scope，示例会额外校验该 scope。
- 如果你拿的是 `demo-spa` 那条 public client 的 token，默认 audience 会对不上，需要把 `AUTH_EXPECTED_AUDIENCE` 改成对应 client ID。
