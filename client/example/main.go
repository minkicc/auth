...
<head>
  ...
  <title>Auth Go OIDC BFF Example</title>
  <style>
    :root {
      --bg: #f4efe6;
  ...
  <div class="shell">
    <section class="hero">
      <article class="card">
        ...
        <h1>Auth OIDC backend callback flow for Go services.</h1>
        <p>
        ...
        <div class="actions">
          <a class="button button-primary" href="/login">Login With Auth</a>
...
func main() {
	...
	log.Printf("Auth OIDC BFF example listening on %s", cfg.ListenAddr)
...
func loadConfig() appConfig {
	return appConfig{
		Issuer:       envOrDefault("AUTH_ISSUER", defaultIssuer),
		ClientID:     envOrDefault("AUTH_CLIENT_ID", defaultClientID),
		ClientSecret: envOrDefault("AUTH_CLIENT_SECRET", defaultClientSecret),
		RedirectURL:  envOrDefault("AUTH_REDIRECT_URL", defaultRedirectURL),
