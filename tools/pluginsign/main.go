...
func runGenKey(args []string) error {
	...
	keyID := fs.String("key-id", "auth-dev", "trusted signer id")
...
func runSign(args []string) error {
	...
	fs.SetOutput(os.Stdout)
	manifestPath := fs.String("manifest", "", "path to auth-plugin.yaml")
	privateKeyPath := fs.String("private-key", "", "path to ed25519 private key PEM")
	keyID := fs.String("key-id", "auth-dev", "trusted signer id")
	out := fs.String("out", "", "output signature path (defaults to manifest dir/auth-plugin.sig)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	if signaturePath == "" {
		signaturePath = filepath.Join(filepath.Dir(*manifestPath), "auth-plugin.sig")
	}
...
func runVerify(args []string) error {
	...
	fs.SetOutput(os.Stdout)
	manifestPath := fs.String("manifest", "", "path to auth-plugin.yaml")
	signaturePath := fs.String("signature", "", "path to auth-plugin.sig")
...
func printRootUsage() {
	fmt.Println("Usage:")
	fmt.Println("  go run ./pluginsign genkey -key-id auth-dev -out-private ./plugin-signing.key.pem -out-public ./plugin-signing.pub")
	fmt.Println("  go run ./pluginsign sign -manifest ../examples/plugins/http-claims-action/auth-plugin.yaml -private-key ./plugin-signing.key.pem -key-id auth-dev")
	fmt.Println("  go run ./pluginsign verify -manifest ../examples/plugins/http-claims-action/auth-plugin.yaml -signature ../examples/plugins/http-claims-action/auth-plugin.sig -public-key-file ./plugin-signing.pub")
}
