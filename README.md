...
Auth can expose a SCIM 2.0 Users endpoint so enterprise directories such as Okta, Entra ID, or Google Workspace can provision users into an organization.
...
The first SCIM version supports Users only. It creates or updates Auth users, links them through `external_identities` with `provider_type=scim`, and syncs organization membership status plus lightweight role names. `DELETE /Users/:id` and `active=false` disable the Auth user and mark the organization membership as disabled.
