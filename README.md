...
When a user has an active organization membership and the downstream OIDC client requests `profile`, Auth can include `org_id`, `org_slug`, and `org_roles` in the ID Token and `/oauth2/userinfo`.
...
Providers created from the admin console are stored in the database, their client secret is not echoed back by the admin API, and saving changes triggers an in-process reload so the enterprise login routes become available immediately without restarting Auth.
...
If a downstream OIDC client already knows the user's work email, it can pass `login_hint=user@example.com` to `/oauth2/authorize`. Auth now forwards that hint to the login page and automatically triggers Enterprise OIDC discovery from it.

If the downstream OIDC client only knows the organization domain, it can instead pass `domain_hint=example.com` to `/oauth2/authorize`. Auth forwards that hint too and performs domain-based Enterprise OIDC discovery automatically.
...
When CIAM/IAM organization data exists and the downstream OIDC client requests `profile`, Auth can also include `org_id`, `org_slug`, and `org_roles` in the ID Token and `/oauth2/userinfo`.
