...
Plugin install, enable, disable, replace, and uninstall operations are written to `auth-plugin.audit.jsonl` in the plugin directory. Replace/uninstall entries keep package fingerprints for rollback investigation. The admin console reads the latest entries through `GET /admin-api/plugins/audit`.
