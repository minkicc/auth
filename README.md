...
- `quickstart/docker-compose.yml` currently uses the prebuilt image `example/auth:latest`, which is convenient for evaluation and demos.
...
On this branch, Auth is OIDC-first. The legacy flow remains as a migration path, but new integrations should use OIDC.
...
This is the easiest way if you already have an application and want Auth to own the login UI.

#### Flow

1. Redirect the user to the Auth login page:

```text
GET /login?client_id=myapp&redirect_uri=https://your-app.example.com/auth/callback
```

2. The user completes login in Auth.
3. Auth redirects the browser back to your `redirect_uri` with a one-time `code`.
...
import (
    ...
    auth "example.com/auth/client/auth"
)
...
### Pattern 2: call Auth APIs from your own frontend
