...
func newTokenSigner(cfg config.OIDCConfig) (*tokenSigner, error) {
	...
	if keyPEM == "" {
		...
		return &tokenSigner{privateKey: key, publicKey: &key.PublicKey, keyID: defaultString(cfg.KeyID, "auth-dev")}, nil
	}
	...
	if err != nil {
		...
		return &tokenSigner{privateKey: pkcs1Key, publicKey: &pkcs1Key.PublicKey, keyID: defaultString(cfg.KeyID, "auth")}, nil
	}
