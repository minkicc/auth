...
func (h *SCIMHandler) handleResourceTypes(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		...
		"Resources": []gin.H{{
			...
			"description": "Auth provisioned user",
		}, {
			"id":          "Group",
			"name":        "Group",
			"endpoint":    "/Groups",
			"schema":      scimGroupSchema,
			"description": "Auth provisioned organization group",
		}},
	})
}
...
func (h *SCIMHandler) handleSchemas(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		...
		"Resources": []gin.H{{
			"id":          scimUserSchema,
			"name":        "User",
			"description": "SCIM core user schema subset supported by Auth",
		}, {
			"id":          scimGroupSchema,
			"name":        "Group",
			"description": "SCIM core group schema subset supported by Auth",
		}},
	})
}
...
func (h *SCIMHandler) authenticateSCIMClient() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			c.Header("WWW-Authenticate", `Bearer realm="auth-scim"`)
