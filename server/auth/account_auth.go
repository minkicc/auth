...
type UserRoleMapping struct {
	...
	Role      UserRole `gorm:"primarykey;size:20"`
	CreatedAt time.Time
}
...
type User struct {
	...
	Roles         []UserRole     `json:"roles" gorm:"many2many:user_role_mappings"`
}
...
func (a *AccountAuth) AssignRole(userID uint, role UserRole) error {
	...
	return a.db.Create(mapping).Error
}

// RemoveRole 移除角色
func (a *AccountAuth) RemoveRole(userID uint, role UserRole) error {
	return a.db.Where("user_id = ? AND role = ?", userID, role).
