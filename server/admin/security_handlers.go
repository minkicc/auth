...
func (s *AdminServer) handleExportSecurityAudit(c *gin.Context) {
	...
	c.Header("X-Auth-Export-Total", strconv.FormatInt(total, 10))
	c.Header("X-Auth-Export-Limit", strconv.Itoa(securityAuditExportMaxRows))
	if truncated {
		c.Header("X-Auth-Export-Truncated", "true")
	}
...
func (s *AdminServer) handleDownloadSecurityAuditExportJob(c *gin.Context) {
	...
	c.Header("X-Auth-Export-Total", strconv.FormatInt(record.TotalCount, 10))
	c.Header("X-Auth-Export-Limit", strconv.Itoa(securityAuditExportMaxRows))
	if record.Truncated {
		c.Header("X-Auth-Export-Truncated", "true")
	}
