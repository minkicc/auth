...
func TestRuntimePreviewZipReportsReplaceImpact(t *testing.T) {
	...
	archiveV1 := buildPluginArchive(t, map[string]string{
		"claims-http/auth-plugin.yaml": configurableClaimsHTTPManifest("https://actions.example.com/claims"),
	})
	...
	archiveV2 := buildPluginArchive(t, map[string]string{
		"claims-http/auth-plugin.yaml": configurableClaimsHTTPManifestV("0.2.0", "https://actions.example.com/claims"),
	})
