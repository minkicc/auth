...
func TestRuntimeInstallsVerifiedSignedPluginAndRunsHooks(t *testing.T) {
	...
	manifestPath := "claims-http/auth-plugin.yaml"
	...
	archive := buildPluginArchive(t, map[string]string{
		...
		"claims-http/auth-plugin.sig": signature,
...
func TestRuntimeRejectsUnsignedPluginWhenSignatureRequired(t *testing.T) {
	...
	archive := buildPluginArchive(t, map[string]string{
		"claims-http/auth-plugin.yaml": claimsHTTPManifest("https://actions.example.com/claims"),
	})
...
audit_sink:
  ...
  resource_types:
    - oidc_client
`
	if err := os.WriteFile(filepath.Join(pluginDir, "auth-plugin.yaml"), []byte(manifest), 0o644); err != nil {
...
func TestRuntimeAuditSinkCanReceiveLifecycleEvents(t *testing.T) {
	...
	sinkServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("X-Auth-Audit-Event") != "lifecycle" {
			t.Fatalf("unexpected audit event header: %q", r.Header.Get("X-Auth-Audit-Event"))
		}
		var payload struct {
			PluginID  string             `json:"plugin_id"`
			Event     string             `json:"event"`
...
func TestRuntimeReloadRejectsTamperedSignedPlugin(t *testing.T) {
	...
	archive := buildPluginArchive(t, map[string]string{
		"claims-http/auth-plugin.yaml": manifest,
		"claims-http/auth-plugin.sig":  signature,
	})
	...
	if err := os.WriteFile(filepath.Join(pluginDir, "auth-plugin.yaml"), tamperedManifest, 0o644); err != nil {
...
func TestRuntimeRejectsDisallowedHTTPActionHost(t *testing.T) {
	...
	archive := buildPluginArchive(t, map[string]string{
		"claims-http/auth-plugin.yaml": claimsHTTPManifest("https://evil.example.com/claims"),
	})
	if _, err := runtime.InstallZip("claims-http.zip", archive, false); err == nil {
