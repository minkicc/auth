...
现在后台不再单独维护管理员用户名和密码，而是直接复用当前浏览器里的 Auth 主站登录会话。配置里的 `auth_admin.user_ids` 属于运维维护的只读管理员；登录后，管理员用户会在主站个人信息页看到进入后台的按钮。日常需要新增或删除的管理员，可以直接在后台 `Settings` 页里管理，这部分数据会存进数据库。
