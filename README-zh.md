...
Auth 在这条分支上优先推荐标准 OIDC 接入，旧协议作为过渡方案保留。
...
适合你已经有业务系统，希望把登录完全交给 Auth。

#### 流程

1. 业务系统把用户跳转到 Auth 登录页：
...
2. 用户在 Auth 完成登录。
3. Auth 会把用户带回你的 `redirect_uri`，并附带一个一次性 `code`。
...
这个模式下，登录页、注册页、第三方登录按钮都由 Auth 自己维护，你的业务系统只处理跳转和回调。
