/*
 * Copyright (c) 2025 Minki Technology (https://minki.cc)
 * Licensed under the MIT License.
 */

export default {
  common: {
    loading: '加载中...',
    registering: '注册中...',
    sending: '发送中...',
    secondsRemaining: '{seconds}秒',
    verificationCode: '验证码',
    phoneNumber: '手机号',
    or: '或',
    error: '错误',
    success: '成功',
    confirm: '确认',
    cancel: '取消',
    submit: '提交',
    back: '返回',
    next: '下一步',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    view: '查看',
    refresh: '刷新',
    logout: '退出登录',
    language: '语言',
    email: '邮箱',
    password: '密码',
    confirmPassword: '确认密码',
    phone: '手机号',
    nickname: '昵称',
    username: '用户名',
    code: '验证码',
    invitationCode: '邀请码（可选）',
    user: '用户',
  },
  auth: {
    login: '登录',
    register: '注册',
    forgotPassword: '忘记密码',
    resetPassword: '重置密码',
    accountLogin: '账号登录',
    emailLogin: '邮箱登录',
    phoneLogin: '手机登录',
    passwordLogin: '密码登录',
    codeLogin: '验证码登录',
    accountRegister: '账号注册',
    emailRegister: '邮箱注册',
    phoneRegister: '手机注册',
    googleLogin: '使用 Google 账号登录',
    wechatLogin: '使用微信登录',
    loginWithGoogle: '使用 Google 登录',
    loginWithWechat: '使用微信登录',
    loginWithEnterprise: '使用 {name} 登录',
    enterpriseEmailLogin: '企业邮箱登录',
    enterpriseEmailPlaceholder: '请输入您的企业邮箱',
    enterpriseEmailLoginHint: '输入企业邮箱后，系统会按已验证域名自动发现对应的企业登录源。',
    enterpriseDiscoveryMatched: '已识别到 {name} 的企业登录方式，正在跳转...',
    enterpriseDiscoveryMultiple: '已识别到 {name} 的多个企业登录方式，请选择一个继续。',
    enterpriseDirectoryLogin: '企业目录登录',
    enterpriseDirectoryLoginHint: '当前企业身份源使用 LDAP/AD，请输入目录账号和密码继续。',
    enterpriseDirectoryUsernamePlaceholder: '请输入目录用户名、邮箱或工号',
    enterpriseDirectoryPasswordPlaceholder: '请输入目录密码',
    enterpriseDirectorySelected: '已选中 {name}，请输入企业目录账号密码继续。',
    sendCode: '发送验证码',
    resendCode: '重新发送',
    getVerificationCode: '获取验证码',
    loginSuccess: '登录成功',
    registerSuccess: '注册成功',
    logoutSuccess: '退出成功',
    passwordResetSuccess: '密码重置成功',
    loginToYourAccount: '登录您的账号',
    createNewAccount: '创建新账号',
    verifyYourAccount: '验证您的账号',
    enterYourDetails: '请输入您的信息',
    noAccount: '没有账号?',
    haveAccount: '已有账号?',
    agreeToTerms: '我同意服务条款和隐私政策',
    loginOptions: '登录选项',
    registerOptions: '注册选项',
    loadingLoginOptions: '加载登录选项中...',
    rememberMe: '记住我',
    loginToSee: '请登录以查看内容',
    codeSent: '验证码已发送',
    codeResent: '验证码已重新发送',
    inputVerificationCode: '请输入验证码',
    verificationCodeSent: '验证码已发送至您的{type}',
    verificationEmailSent: '验证邮件已发送',
    verificationEmailSentTo: '我们已向 <strong>{email}</strong> 发送了一封验证邮件。',
    pleaseCheckEmail: '请查收邮件并点击验证链接完成注册。',
    notReceivedEmail: '没有收到邮件？',
    checkSpamFolder: '请检查垃圾邮件或促销邮件文件夹',
    confirmEmailCorrect: '确认您输入的邮箱地址正确',
    waitAndCheckAgain: '等待几分钟后再次检查',
    resendVerificationEmail: '重新发送验证邮件',
    useAnotherEmail: '使用其他邮箱',
    verificationEmailResent: '验证邮件已重新发送，请查收',
    goToLogin: '去登录',
    phoneRegisterSuccessMessage: '您的手机号注册成功，可以开始登录使用了。',
    processingWeixinLogin: '正在处理微信登录...',
    backToLogin: '返回登录',
  },
  validation: {
    required: '{field}不能为空',
    minLength: '{field}长度不能少于{min}个字符',
    maxLength: '{field}长度不能超过{max}个字符',
    passwordLength: '密码长度至少为{min}位',
    nicknameLength: '昵称长度应在{min}-{max}个字符之间',
    passwordMatch: '两次输入的密码不一致',
    passwordMismatch: '两次输入的密码不一致',
    invalidEmail: '请输入有效的邮箱地址',
    invalidPhone: '请输入有效的手机号码',
    invalidCode: '请输入有效的验证码',
    verificationCodeFormat: '验证码应为6位数字',
    invalidUsername: '用户名格式不正确',
    usernameExists: '该用户名已被使用',
    emailExists: '该邮箱已被注册',
    phoneExists: '该手机号已被注册',
    agreementRequired: '请阅读并同意用户协议和隐私政策',
  },
  errors: {
    loginFailed: '登录失败，请检查账号和密码',
    registerFailed: '注册失败，请稍后再试',
    emailLoginFailed: '邮箱登录失败，请重试',
    emailRegisterFailed: '邮箱注册失败，请重试',
    phoneRegisterFailed: '手机注册失败，请重试',
    resendVerificationFailed: '重新发送验证邮件失败，请重试',
    sendCodeFailed: '验证码发送失败，请重试',
    unauthorized: '未授权访问',
    forbidden: '禁止访问',
    serverError: '服务器错误，请稍后再试',
    networkError: '网络错误，请检查网络连接',
    unknownError: '未知错误',
    invalidCredentials: '账号或密码错误',
    accountLocked: '账号已被锁定，请联系管理员',
    accountDisabled: '账号已被禁用',
    tooManyAttempts: '尝试次数过多，请稍后再试',
    sessionExpired: '会话已过期，请重新登录',
    googleLoginFailed: 'Google 登录失败',
    wechatLoginFailed: '微信登录失败',
    emailVerificationFailed: '邮箱验证失败',
    phoneVerificationFailed: '手机验证失败',
    codeExpired: '验证码已过期',
    codeSendFailed: '验证码发送失败',
    passwordResetFailed: '密码重置失败',
    fetchProvidersFailed: '获取支持的登录方式失败',
    initLoginPageFailed: '初始化登录页面失败',
    loadLoginOptionsFailed: '加载登录选项失败，请刷新页面重试',
    googleClientIdMissing: '未配置谷歌客户端ID',
    googleSdkLoadFailed: '加载谷歌登录SDK失败',
    wechatUrlFetchFailed: '获取微信登录链接失败',
    phoneLoginFailed: '手机号登录失败，请重试',
    phoneCodeLoginFailed: '验证码登录失败，请重试',
    passwordResetInitFailed: '发起密码重置失败，请重试',
    googleConfigIncomplete: '谷歌登录配置不完整',
    googleCredentialMissing: '谷歌登录失败：未获取到凭证',
    googleLoginProcessFailed: '谷歌登录处理失败',
    renderGoogleButtonFailed: '渲染谷歌登录按钮失败',
    googleServiceLoadFailed: '加载谷歌登录服务失败',
    invalidWeixinCallback: '微信回调参数无效',
    enterpriseOIDCProvidersFailed: '获取企业登录方式失败',
    enterpriseOIDCLoginFailed: '企业登录跳转失败',
    enterpriseOIDCDiscoveryFailed: '企业邮箱发现失败，请稍后再试',
    enterpriseEmailRequired: '请输入企业邮箱',
    enterpriseEmailDomainNotFound: '没有找到与该邮箱域名匹配的企业登录源',
    enterpriseDomainNotFound: '没有找到与该企业域名匹配的企业登录源',
    enterpriseOrganizationInactive: '该企业组织当前不可用，请联系管理员',
    enterpriseOIDCNotConfigured: '该企业已命中，但尚未配置可用的企业登录源',
    enterpriseLDAPLoginFailed: '企业目录登录失败，请检查账号密码或联系管理员',
    enterpriseDirectoryCredentialsRequired: '请输入企业目录账号和密码',
    organizationSelectionLoadFailed: '加载可选组织失败，请稍后重试',
    organizationSelectionFailed: '继续组织授权失败，请重新选择',
  },
  email: {
    verificationTitle: '邮箱验证',
    registrationTitle: '注册 MKAuth',
    verificationEmailTpl: `
  <h2>邮箱验证</h2>
  <p>您好，请点击以下链接验证您的邮箱：</p>
  <p><a href="<%VerifyURL%>">验证邮箱</a></p>
  <p>如果链接无法点击，请复制以下地址到浏览器打开：</p>
  <p><%VerifyURL%></p>
  <p>此链接将在24小时后过期。</p>
  `,
  passwordResetEmailTpl: `
  <h2>密码重置</h2>
  <p>您好，请点击以下链接重置您的密码：</p>
  <p><a href="<%ResetURL%>">重置密码</a></p>
  <p>如果链接无法点击，请复制以下地址到浏览器打开：</p>
  <p><%ResetURL%></p>
  <p>此链接将在24小时后过期。如果您没有请求重置密码，请忽略此邮件。</p>
  `,
  loginNotificationEmailTpl: `
  <h2>登录通知</h2>
  <p>您好，您的账号刚刚在新设备上登录：</p>
  <p>IP地址：<%Ip%></p>
  <p>时间：<%Time%></p>
  <p>如果这不是您本人的操作，请立即修改密码。</p>
  `,
  },
  logs: {
    googleInit: '开始初始化谷歌登录...',
    googleInitComplete: '谷歌登录按钮初始化完成',
    googleInitFailed: '初始化谷歌登录失败:',
    googleButtonRendered: '谷歌登录按钮已渲染',
  },
  notFound: {
    title: '页面未找到',
    description: '抱歉，您访问的页面不存在。',
    backToHome: '返回首页',
  },
  emailVerify: {
    loading: '正在验证您的邮箱...',
    verifyFailed: '验证失败',
    redirecting: '验证成功，正在跳转...',
    resendVerification: '重新发送验证邮件',
    sending: '发送中...',
    register: '重新注册',
    backToLogin: '返回登录',
    resendSuccess: '验证邮件已重新发送，请查收',
    verifySuccess: '验证成功',
    emailVerified: '您的邮箱已验证成功，账号已激活。',
    userId: '用户ID',
    nickname: '昵称',
    enterHome: '进入首页',
    invalidLink: '验证链接无效，缺少验证令牌',
    cannotResend: '无法重新发送验证邮件，请重新注册'
  },
  profile: {
    title: '用户信息',
    subtitle: '当前浏览器已经登录，可以直接查看基础资料。',
    loading: '正在加载用户信息...',
    loadFailed: '加载用户信息失败',
    userId: '用户 ID',
    username: '用户名',
    nickname: '昵称',
    avatar: '头像',
    sessionExpiresAt: '会话过期时间',
    organizationAuthorization: '当前身份上下文',
    organizationAuthorizationHint: '选择平台或企业身份后，MKAuth 会把它作为后续授权的默认上下文。',
    platformIdentity: '平台',
    platformAuthorizationHint: '当前使用平台身份，不进入任何企业组织。',
    activeScopeUpdateFailed: '切换当前身份失败',
    organizationId: '组织 ID',
    organizationSlug: '组织 Slug',
    organizationRoles: '组织角色',
    organizationGroups: '组织组',
    organizationPermissions: '权限键',
    adminPage: '管理页面',
    noAvatar: '暂无头像',
    backToLogin: '返回登录页'
  },
  organizationSelect: {
    eyebrow: '组织上下文',
    loading: '正在加载可选组织...',
    title: '选择这次登录要使用的身份',
    subtitle: '当前账号属于多个组织，也可以继续使用平台/个人身份。选择后，MKAuth 会把本次 OIDC 授权、令牌和 userinfo 固定到对应上下文。',
    clientId: '业务客户端',
    organizationCount: '可选组织',
    identityCount: '可选身份',
    identityType: '身份类型',
    platformInitial: '平',
    platformTitle: '平台 / 个人身份',
    platformSubtitle: '不进入任何企业组织，继续使用平台默认上下文。',
    platformType: '平台身份',
    available: '可用',
    status: '组织状态',
    roles: '组织角色',
    groups: '组织组',
    current: '当前上下文',
    continue: '继续进入这个组织',
    continuePlatform: '继续使用平台身份',
    redirecting: '正在继续授权...',
    emptyTitle: '当前账号没有可用组织',
    emptySubtitle: '如果你本来应该属于某个组织或看到平台身份，请联系管理员检查 OIDC 客户端策略、组织成员关系或目录同步配置。',
    errorTitle: '组织选择暂时不可用'
  }
} 
