...
`quickstart/` 提供了两套可直接体验的 Docker Compose 配置，用来快速启动 Auth 所需的基础依赖，以及一个最小可运行的 OIDC PKCE demo。
...
- `quickstart/docker-compose.release.yml` 会直接拉取 `ghcr.io/example/auth` 镜像，更适合发给其他用户或部署环境使用。
- 两个 compose 文件都会等待 MySQL、Redis、MinIO 健康后再启动 `auth-server`，启动顺序更稳。
