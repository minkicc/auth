...
This example plugin forwards Auth admin security audit events to an HTTPS webhook. It can also opt into authentication lifecycle events.
...
If `secret` or `secret_env` is configured, Auth sends `Authorization: Bearer <token>`.
