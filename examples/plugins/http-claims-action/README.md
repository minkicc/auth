...
The `auth-plugin.yaml` file already contains the runtime `http_action` block, so no extra `plugins.http_actions` entry is required for this local plugin.
...
```bash
export AUTH_HTTP_CLAIMS_SECRET="YOUR_ACTION_BEARER_SECRET"
```
...
Auth sends a JSON payload to the configured URL. The action may return extra `claims` and `metadata`, or deny the flow with `allow: false`.
