...
Auth has already moved beyond a simple authentication service and into an extensible B2B CIAM platform:
...
- access tokens can be revoked before expiry and are rejected by Auth online validation paths
...
> Auth is an extensible B2B CIAM platform that provides enterprise identity federation, organization-aware authentication, inbound SCIM provisioning, downstream OIDC, and plugin-ready identity extension points.
