...
2. Auth validates the archive structure, per-file limits, manifest schema, declared permissions, and optional signature.
3. Auth can also fetch remote plugin catalogs and install ZIP packages directly from a catalog entry or explicit download URL.
4. Auth writes `auth-plugin.state.yaml` beside the manifest to persist enabled or disabled state and the uploaded package SHA-256 fingerprint.
5. Auth appends plugin management operations to `auth-plugin.audit.jsonl` in the plugin directory.
6. Replace and uninstall operations create rollback snapshots under `.auth-plugin-backups`; restore revalidates signature, permissions, and host policy before activation.
