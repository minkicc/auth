...
Auth's current core is an OIDC-first authentication service. The CIAM/IAM path should keep that core small and add extension points around it instead of turning every integration into hard-coded login logic.
...
Identity connectors authenticate users against upstream identity providers and link the upstream subject to Auth's internal `usr_...` user ID.
...
config_schema:
  - key: "url"
    ...
    type: "url"
    required: true
    default: "https://actions.example.com/auth"
  - key: "timeout_ms"
    label: "Timeout (ms)"
    type: "integer"
    default: "3000"
...
config_schema:
  ...
  - key: "secret"
    ...
    sensitive: true
audit_sink:
  url: "https://audit.example.com/auth/events"
  secret_env: "AUTH_AUDIT_WEBHOOK_TOKEN"
  timeout_ms: 3000
  fail_open: true
  resource_types:
    - "oidc_client"
...
`config_schema` makes installed plugins configurable from the admin console. Values are stored in `auth-plugin.state.yaml`, sensitive fields are not echoed back by the admin API, and local HTTP Action plugins can use saved config to override `url`, `secret`, `secret_env`, `timeout_ms`, and `fail_open`.

Runtime behavior:

1. Installing a ZIP package copies it into the first configured plugin directory.
2. Auth can preview an uploaded ZIP before installation, returning the parsed manifest, package SHA-256, signature status, replacement impact, and config keys that would be preserved or dropped.
3. Auth validates the archive structure, per-file limits, manifest schema, declared permissions, and optional signature.
4. Auth can also fetch remote plugin catalogs and install ZIP packages directly from a catalog entry or explicit download URL.
5. Auth writes `auth-plugin.state.yaml` beside the manifest to persist enabled or disabled state and the uploaded package SHA-256 fingerprint.
6. Auth appends plugin management operations to `auth-plugin.audit.jsonl` in the plugin directory.
7. Replace and uninstall operations create rollback snapshots under `.auth-plugin-backups`; restore revalidates signature, permissions, and host policy before activation.
...
- `tools/pluginsign sign` signs the raw manifest content into `auth-plugin.sig`
...
Claim mappers transform Auth user, organization, membership, and group data into OIDC claims. The first useful claims are:
...
Provisioning connectors synchronize users and group membership from external directories. The first target should be inbound SCIM, where enterprise directories push users into Auth.
...
- `external_identities`: upstream provider subject to Auth user mapping.
...
2. Creates Auth users with random unusable local passwords.
...
5. Maps `active=false` and `DELETE /Users/:id` to disabled Auth users and disabled organization memberships.
...
This is still a lightweight role-mapping layer, not a full group/RBAC policy engine. Auth now exposes first-class `org_groups` claims and delegated manual group administration, but it still does not provide full policy enforcement.
...
The login endpoint accepts an optional `return_uri` query parameter. Use a relative URL such as `/oauth2/authorize?...` when enterprise SSO should resume an existing downstream OIDC authorization request after Auth creates the browser session.
...
2. Auth looks up a verified record in `organization_domains`.
...
7. When a downstream OIDC client sends `login_hint`, Auth forwards it into the login page and reuses it to trigger HRD automatically.
8. When a downstream OIDC client sends `domain_hint`, Auth can skip the email requirement and trigger HRD directly from the hinted organization domain.
...
6. Reuses the existing Auth `oidc_session` browser session mechanism.
...
The login endpoint accepts an optional `return_uri` query parameter, just like Enterprise OIDC, so enterprise SSO can resume an existing downstream OIDC authorization request after Auth creates the browser session.

Enterprise SAML behavior:

1. Loads IdP metadata from `idp_metadata_url` or inline `idp_metadata_xml`.
2. Generates SP defaults for metadata and ACS from the public Auth base URL when explicit values are omitted.
...
7. Reuses the existing Auth `oidc_session` browser session mechanism after SAML login succeeds.
...
3. Auth optionally binds with `bind_dn` and `bind_password`, searches under `base_dn` with `user_filter`, and then binds as the matched user DN to verify credentials.
...
5. Auth links `(provider_type=ldap, provider_id=:slug, subject=...)` into `external_identities`.
...
10. Reuses the existing Auth `oidc_session` browser session mechanism after directory login succeeds.

## Organization Claims

When a user has an active organization membership and the downstream OIDC client requests the `profile` scope, Auth adds these claims to the ID Token and `/oauth2/userinfo` response:
...
By default, Auth selects the earliest active organization membership. Downstream OIDC clients can now override that by sending `org_hint=<organization_id_or_slug>` to `/oauth2/authorize`. A future version should add an interactive organization chooser for end users who belong to multiple organizations.
By default, Auth selects the earliest active organization membership. Downstream OIDC clients can now override that by sending `org_hint=<organization_id_or_slug>` to `/oauth2/authorize`. If no `org_hint` is provided and the browser session belongs to multiple active organizations, Auth redirects the user to `/select-organization` so the user can choose the organization interactively. When the downstream client also sends `prompt=none`, Auth returns `interaction_required` instead of showing the chooser.
