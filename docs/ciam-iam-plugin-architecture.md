...
5. When a downstream OIDC client sends `login_hint`, Auth forwards it into the login page and reuses it to trigger HRD automatically.
6. When a downstream OIDC client sends `domain_hint`, Auth can skip the email requirement and trigger HRD directly from the hinted organization domain.
