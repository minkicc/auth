...
Create an R2 S3 API token with object read/write access to that bucket, then configure Auth:
...
Cloudflare Email Routing only handles inbound routing/forwarding. For Auth email login and registration verification codes, use Cloudflare Email Service after onboarding your sending domain in Cloudflare DNS.
...
Create an API token with permission to send emails, expose it to the container through `AUTH_CLOUDFLARE_EMAIL_API_TOKEN`, and make sure the `from` address belongs to an onboarded sending domain.
...
- Your auth domain must route to the Auth HTTPS service.
